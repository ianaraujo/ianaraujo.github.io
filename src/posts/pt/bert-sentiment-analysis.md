...
Se você estiver interessado em ver o projeto completo, junto dos códigos, ele está disponível no meu [GitHub](https://github.com/ianaraujo/bert-finance-sentiment).
...
Um termômetro quantitativo muito conhecido é o ["Fear & Greed Index"](https://edition.cnn.com/markets/fear-and-greed) da CNN, que mede o sentimento dos investidores americanos e varia entre 0 (medo) e 100 (ganância).
...
Este trabalho foi fortemente inspirado pelo artigo do [Lucas Leme](https://www.linkedin.com/in/lucas-leme-santos/), que usou notícias de sites de finanças, como Valor Econômico e Infomoney, para treinar uma modelo capaz de prever o sentimento do mercado.
...
A primeira etapa do trabalho, portanto, foi coletar essas cartas de gestores de recursos diretamente dos seus sites. Para isso, utilizei a biblioteca [BeautifulSoup](https://www.crummy.com/software/BeautifulSoup/bs4/doc/) para fazer o *scraping* dos sites das gestoras e a extração do texto dos PDFs.
...
Os modelos de linguagem são uma classe de modelos de aprendizado profundo projetados para compreender e gerar texto. Eles são baseados na arquitetura de *transformers*, que foi introduzida no artigo ["Attention is All You Need"](https://arxiv.org/abs/1706.03762) em 2017.
...
Da mesma forma que o treinamento anterior, utilizei um servidor remoto com uma instância de GPU e o código para o treinamento está disponível no [repositório](https://github.com/ianaraujo/bert-finance-sentiment) do projeto. Utilizei a biblioteca [PEFT](https://huggingface.co/docs/peft/en/index) do Hugging Face e, ao todo, o modelo foi treinado em somente 1,181,955 parâmetros, pouco mais de 1% do número de parâmetros do modelo base.

Ao final do treinamento, é possível combinar o modelo base com o adaptador treinado, criando um novo modelo que pode ser usado para realizar a tarefa de classificação de texto. O modelo final pode ser acessado no meu perfil do [Hugging Face Hub](https://huggingface.co/ianaraujo/bert-portuguese-asset-management-sentiment).
...
Agradeço quem leu até aqui e espero que tenha gostado do artigo. Se você tiver alguma dúvida ou sugestão, fique à vontade para entrar em contato comigo através do meu e-mail, [LinkedIn](https://www.linkedin.com/in/ianaraujo/) ou [Twitter](https://twitter.com/ianaraujo_).
